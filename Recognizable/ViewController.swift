...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        guard let selectedModel = try? VNCoreMLModel(for: modelJan().model) else {
    ...
    func classificationCompleteHandler(request: VNRequest, error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            if (topPredictionScore != nil && topPredictionScore! > 0.008) {
                if (topPredictionName == "Pistol") { symbol = "🔫" }
            }
            if (topPredictionScore != nil && topPredictionScore! > 0.01) {
                if(topPredictionScore! > 0.90){
                    if (topPredictionName == "Faust") { symbol = "👊" }
                }
                if (topPredictionName == "DaumenHoch") { symbol = "👍" }
                if (topPredictionName == "Finger") { symbol = "🔥" }
